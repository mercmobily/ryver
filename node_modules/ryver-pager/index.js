...
eventEC.onCollect( 'filter', function( cb ){
  ...
  var f = function( fileData, cb ){
    ...
    if( config.pageSeparator == '' ){
      ryver.log( "File ignored by plugin pager as fileSeparator is not defined" );
      return cb( null, fileData);
    }

    var pieces = fileData.contents.split( config.pageSeparator );

    // This filter will only work on text files (of any kind)
    if( pieces.length == 1 ){
      ryver.log( "File ignored by plugin pager as there is no fileSeparator" );
      return cb( null, fileData);
    }
    ...
    // Make up the pageFileName, from config or a sane default
