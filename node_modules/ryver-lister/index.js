...
*
When a file is being filtered, it will need to:

- Delete the entry ANYWHERE in listData (_ALL_ and sub-positions)
- (Entry will be re-added by filtering)

- Refilter all files containing makeListLatestVars (showing lists):
  * Set skipMe as true, involvedOnes = {}
  * Copy associationCache[ originURL ] over to previousAssociationCache[ originURL ]
  * Add list of tags/categories associated to that originURL to associationCache[ originURL ];
  * Make involvedOnes, as associationCache[ originURL ] + previousAssociationCache[ originURL]
  * Filter all files (from _ALL_) with makeListLatestVars containing at least one elem in involvedOnes
  * Run delayedFilters
  * Set skipMe as false

- Refilter lists
  * Add parameter to makeLists so that you can pass it involvedOnes, which if present will filter
  * Run makeLists from afterDelayedPostProcess

DONE!
*/

// lister -- delete (TODO), and (re?)place, entry in listData
...
eventEC.onCollect( 'afterDelayedPostProcess', function( cb ){
  ...
  // Return the function just defined as the filter
  cb( null, { name: 'XXXXXXXX', executor: f } );
