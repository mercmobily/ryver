      ...
      function makeList( listName, c, data, cb ){
        ...
        async.whilst(
          ...
          function( cb ){
            ...
            if( pageNumber == 1 ) fileName = config.listMainPageName || 'index';
            else fileName = ( config.listNumberPageName || 'index_{{number}}').replace( '{{number}}', pageNumber );
            ryver.log( "fileName resolved as:", fileName );

            // TODO: Create variabels ready for the page

            ryver.log("Making fileData with:", filePath, fileName, templateFileExt, ryver.getMainInfo() );

            ryver.makeFileData( filePath, fileName + templateFileExt, listTemplateAsBuffer, ryver.getMainInfo(), function( err, fileData ){
              if( err ) return cb( err );

              ryver.vlog("Created fileData:" );
              ryver.vlog( fileData );
