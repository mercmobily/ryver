...
eventEC.onCollect( 'filter', function( cb ){
  ...
  var f = function( fileData, cb ){
    ...
    ryver.readFile( mainPath, backupPath, landing, function( err, landingContentsAsBuffer, usedBackup ){
      ...
      ryver.makeFileData( fileData.system.originMasterFileURL, s.filePath, s.fileName + landingExt, landingContentsAsBuffer, ryver.cloneObject( fileData.initialInfo ), function( err, landingFileData ){
        if( err ) return cb( err );

        // Add the landing file as origin for this file, since -- if it changes -- the file will
        // also need to change
        fileData.system.originFileURLs.push( p.join( originPath, landing ) );
        landingFileData.system.originFileURLs.push( p.join( originPath, landing ) );
